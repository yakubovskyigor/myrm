...
def test_rm(fake_tree, caplog, mocker):
    ...
    with caplog.at_level(logging.INFO):
        rmlib.rm(path, dry_run=False)

    logger_mock.info.assert_called_with("Item '%s' was removed without errors.", path)
    assert not os.path.exists(path)


def test_rm_with_dry_run(fake_tree, caplog, mocker):
    logger_mock = mocker.patch("myrm.rmlib.logger")
    path = fake_tree[1]

    with caplog.at_level(logging.INFO):
        rmlib.rm(path, dry_run=True)
...
def test_rmdir(fake_tree, mocker, caplog):
    ...
    with caplog.at_level(logging.INFO):
        rmlib.rmdir(path, dry_run=False)
...
def test_mkdir(caplog, mocker, fs):
    ...
    with caplog.at_level(logging.INFO):
        rmlib.mkdir(path, dry_run=False)
...
def test_mkdir_with_dry_run(caplog, mocker, fs):
    logger_mock = mocker.patch("myrm.rmlib.logger")
    path = "dir"

    with caplog.at_level(logging.INFO):
        rmlib.mkdir(path, dry_run=True)
...
def test_mv(caplog, mocker, fs):
    ...
    with caplog.at_level(logging.INFO):
        rmlib.mv(src, dst, dry_run=False)
...
def test_mv_with_dry_run(caplog, mocker, fs):
    logger_mock = mocker.patch("myrm.rmlib.logger")
    src = "11.txt"
    fs.create_file(src)
    dst = "2.txt"
...
def test_mvdir(fake_tree, mocker, caplog):
    ...
    with caplog.at_level(logging.INFO):
        rmlib.mvdir(src, dst, dry_run=False)
...
def test_mvdir_with_dry_run(fake_tree, mocker, caplog):
    ...
    with caplog.at_level(logging.INFO):
        rmlib.mvdir(src, dst, dry_run=True)
